...
namespace data_structures_assignment6
{
    ...
    // of the target were removed.
    {
        ...
        binary_tree_node<Item>* oldroot_ptr;
        typename bag<Item>::size_type answer = 0;
        ...
        {   // Continue looking in the left subtree
            /* STUDENT WORK */
            answer += bst_remove_all(root_ptr->left(), target);
        ...
        {   // Continue looking in the right subtree
            /* STUDENT WORK */
            answer+= bst_remove_all(root_ptr->right(), target);
    ...
    // Header file used: bintree.h
    {
        ...
        {   // Move down the tree and add a new leaf:
            cursor = root_ptr;
            bool done = false; // true once item is inserted
            while (!done)
            {
                if (entry <= cursor->data())
                {
                    // try inserting in the left subtree
                    if (cursor->left() == NULL)
                    {
                ...
                {
                    // try inserting in the right subtree
    ...
    typename bag<Item>::size_type bag<Item>::count(const Item& target) const
    {
        ...
        while (cursor != NULL)
        {
            ...
            {
                answer++;
                // because of the way I implemented insert,
                // items with equal value are on the left subtree
                cursor = cursor->left();
            }
        }
